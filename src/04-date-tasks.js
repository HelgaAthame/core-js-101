...
function timeSpanToString(startDate, endDate) {
  ...
  return `${d.getUTCHours().toString().padStart(2, '0')}:${d
    .getMinutes().toString().padStart(2, '0')}:${d
    .getSeconds().toString().padStart(2, '0')}.${d
    .getMilliseconds().toString().padStart(3, '0')}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const a = Math.abs(60 * h - 11 * m) / 360;
  return a === 1 ? a * Math.PI : Math.min(a, Math.abs(1 - a)) * Math.PI;
}
